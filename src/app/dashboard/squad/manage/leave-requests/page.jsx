...
import { ArrowLeft, LogOut, Check, X, Clock, CheckCircle } from "lucide-react";
...
const statusConfig = {
    PENDING: { label: "Wants to Leave", color: "text-orange-400 bg-orange-400/10 border-orange-400/20" },
...
export default function LeaveRequestsPage() {
    ...
    const [acting, setActing] = useState(null); // req._id being actioned
    ...
    const handle = async (reqId, action) => {
        setActing(reqId);
        try {
            if (action === "approve") await approveLeaveRequest(reqId);
            else await rejectLeaveRequest(reqId);
            setRequests((prev) =>
                prev.map((r) =>
                    r._id === reqId
        ...
        <div className="max-w-3xl mx-auto space-y-6 pb-10">
            ...
            {!loading && requests.length === 0 && (
                <div className="rounded-2xl border border-dashed border-zinc-800 p-16 text-center">
                    <LogOut size={36} className="mx-auto text-zinc-700 mb-3" />
                    <p className="text-zinc-400 font-medium">No leave requests</p>
                    <p className="text-zinc-600 text-sm mt-1">Players who request to leave your squad will appear here</p>
                </div>
            )}
                ...
                <section className="space-y-3">
                    ...
                    <div className="space-y-3">
                        {pending.map((req) => (
                            <div
                                key={req._id}
                                className="flex items-center gap-4 rounded-2xl bg-zinc-900/60 border border-orange-400/10 p-4 hover:border-orange-400/20 transition"
                            >
